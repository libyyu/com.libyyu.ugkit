...
namespace UGKit.Asset.Runtime
{
    ...
    public partial class AssetManager : GameFrameworkModule, IAssetManager
    {
        ...
        public async UniTask<bool> DoesStreamingAssetsPackageExists(string packageName)
        {
#if UNITY_EDITOR
        ...
        #region At Editor
        private bool _PackageExistsAtEditor(string packageName)
        {
            var yooDefaultFolderName = YooAssetSettingsData.GetDefaultYooFolderName();
            var dirPath = Path.Combine(Application.streamingAssetsPath, yooDefaultFolderName, packageName);

            // 直接檢查文件夾是否存在
            bool exists = Directory.Exists(dirPath);
            if (exists)
            {
                Log.Info($"【Try Query Builtin-Package】Package exists. Package: {packageName}");
                return true;
            }

            Log.Warning($"【Try Query Builtin-Package】Package doesn't exist. Package: {packageName}");
            return false;
        }
        #endregion

        #region At Runtime
        private async UniTask<bool> _PackageExistsAtRuntime(string packageName)
        {
            Log.Info($"Begin _PackageExistsAtRuntime {packageName}");
            ...
            if (!string.IsNullOrEmpty(packageName))
            {
                ...
                if (exists)
                {
                    Log.Info($"【Try Query Builtin-Package】Search succeeded (Package exists). Package: {packageName}");
                    return true;
                }
        ...
        /// <returns></returns>
        private async UniTask<bool> _TryQueryBuiltinPackageFileExists(string packageName, string fileName)
        {
            // Builtin (StreamingAssets)
            string builtinPackagePath = YooAssetBridge.GetBuiltinPackagePath(packageName);
            string url = Path.Combine(builtinPackagePath, fileName).Replace('\\', '/');
            Log.Info($"Try query builtin Package: {packageName}, URL: {url}");
            // Convert url to www path
            return await _WebRequestPartialBytes(YooAssetBridge.DownloadSystemHelper.ConvertToWWWPath(url));
        }

        private async UniTask<bool> _WebRequestPartialBytes(string url)
        {
            ...
            try
            {
                ...
                {
                    Log.Warning($"【Try Query Builtin-Package】Package doesn't exist (Built-in package not found). URL: {url}");
                    request.Dispose();
                    return false;
                }
                ...
                {
                    Log.Info($"【Try Query Builtin-Package】Package exists (Built-in package found). Code: {request.responseCode}, PartialBytes: {request.downloadedBytes}, URL: {url}");
                    request.Dispose();
                    return true;
                }
            }
            ...
            {
                Log.Warning($"【Try Query Builtin-Package】Request timed out. URL: {url}");
                request.Dispose();
                return false;
            }
            catch (Exception ex)
            {
                Log.Warning($"【Try Query Builtin-Package】Request failed (Package doesn't exist) The package may not exist. URL: {url}, Exception: {ex}");
                request.Dispose();
                return false;
            }
